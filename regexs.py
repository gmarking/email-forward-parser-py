...
SUBJECT = [
    ...
    re.compile(r"(?m)^Vs:(.*)"),  # Outlook Live / 365 (no)
    # Outlook Live / 365 (pl), New Outlook 2019 (pl)
    re.compile(r"(?m)^PD:(.*)"),
    re.compile(
        r"(?m)^ENC:(.*)"
    ),  # Outlook Live / 365 (pt-br), New Outlook 2019 (pt-br)
    re.compile(r"(?m)^Redir.:(.*)"),  # Outlook Live / 365 (ro)
    # Outlook Live / 365 (sv), New Outlook 2019 (sv)
    re.compile(r"(?m)^VB:(.*)"),
    re.compile(r"(?m)^VL:(.*)"),  # New Outlook 2019 (fi)
    re.compile(r"(?m)^Videresend:(.*)"),  # New Outlook 2019 (no)
    re.compile(r"(?m)^İLT:(.*)"),  # New Outlook 2019 (tr)
    re.compile(
        r"(?m)^Fwd:(.*)"
...
SEPARATOR = [
    re.compile(r"^>?\s*Begin forwarded message\s?:"),  # Apple Mail (en)
    re.compile(r"(?m)^>?\s*Začátek přeposílané zprávy\s?:"),  # Apple Mail (cs)
    # Apple Mail (da)
    re.compile(r"(?m)^>?\s*Start på videresendt besked\s?:"),
    re.compile(
        r"(?m)^>?\s*Anfang der weitergeleiteten Nachricht\s?:"
    ),  # Apple Mail (de)
    # Apple Mail (es)
    re.compile(r"(?m)^>?\s*Inicio del mensaje reenviado\s?:"),
    re.compile(r"(?m)^>?\s*Välitetty viesti alkaa\s?:"),  # Apple Mail (fi)
    re.compile(r"(?m)^>?\s*Début du message réexpédié\s?:"),  # Apple Mail (fr)
    # Apple Mail iOS (fr)
    re.compile(r"(?m)^>?\s*Début du message transféré\s?:"),
    # Apple Mail (hr)
    re.compile(r"(?m)^>?\s*Započni proslijeđenu poruku\s?:"),
    re.compile(r"(?m)^>?\s*Továbbított levél kezdete\s?:"),  # Apple Mail (hu)
    re.compile(r"(?m)^>?\s*Inizio messaggio inoltrato\s?:"),  # Apple Mail (it)
    re.compile(r"(?m)^>?\s*Begin doorgestuurd bericht\s?:"),  # Apple Mail (nl)
    re.compile(r"(?m)^>?\s*Videresendt melding\s?:"),  # Apple Mail (no)
    # Apple Mail (pl)
    ...
    re.compile(r"(?m)^>?\s*Vidarebefordrat mejl\s?:"),  # Apple Mail (sv)
    re.compile(r"(?m)^>?\s*İleti başlangıcı\s?:"),  # Apple Mail (tr)
    # Apple Mail (uk)
    re.compile(r"(?m)^>?\s*Початок листа, що пересилається\s?:"),
    re.compile(
        r"(?m)^\s*-{8,10}\s*Forwarded message\s*-{8,10}\s*"
    ),  # Gmail (all locales), Missive (en), HubSpot (en)
    re.compile(r"(?m)^\s*_{32}\s*$"),  # Outlook Live / 365 (all locales)
    re.compile(
        r"(?m)^\s?Dne\s?.+\,\s?.+\s*[\[|<].+[\]|>]\s?napsal\(a\)\s?:"
    ),  # Outlook 2019 (cz)
    re.compile(
        r"(?m)^\s?D.\s?.+\s?skrev\s?\".+\"\s*[\[|<].+[\]|>]\s?:"
    ),  # Outlook 2019 (da)
    re.compile(
        r"(?m)^\s?Am\s?.+\s?schrieb\s?\".+\"\s*[\[|<].+[\]|>]\s?:"
    ),  # Outlook 2019 (de)
    re.compile(
        r"(?m)^\s?On\s?.+\,\s?\".+\"\s*[\[|<].+[\]|>]\s?wrote\s?:"
    ),  # Outlook 2019 (en)
    re.compile(
        r"(?m)^\s?El\s?.+\,\s?\".+\"\s*[\[|<].+[\]|>]\s?escribió\s?:"
    ),  # Outlook 2019 (es)
    re.compile(
        r"(?m)^\s?Le\s?.+\,\s?«.+»\s*[\[|<].+[\]|>]\s?a écrit\s?:"
    ),  # Outlook 2019 (fr)
    re.compile(
        r"(?m)^\s?.+\s*[\[|<].+[\]|>]\s?kirjoitti\s?.+\s?:"
    ),  # Outlook 2019 (fi)
    re.compile(
        r"(?m)^\s?.+\s?időpontban\s?.+\s*[\[|<|(].+[\]|>|)]\s?ezt írta\s?:"
    ),  # Outlook 2019 (hu)
    re.compile(
        r"(?m)^\s?Il giorno\s?.+\s?\".+\"\s*[\[|<].+[\]|>]\s?ha scritto\s?:"
    ),  # Outlook 2019 (it)
    re.compile(
        r"(?m)^\s?Op\s?.+\s?heeft\s?.+\s*[\[|<].+[\]|>]\s?geschreven\s?:"
    ),  # Outlook 2019 (nl)
    re.compile(
        r"(?m)^\s?.+\s*[\[|<].+[\]|>]\s?skrev følgende den\s?.+\s?:"
    ),  # Outlook 2019 (no)
    re.compile(
        r"(?m)^\s?Dnia\s?.+\s?„.+”\s*[\[|<].+[\]|>]\s?napisał\s?:"
    ),  # Outlook 2019 (pl)
    re.compile(
        r"(?m)^\s?Em\s?.+\,\s?\".+\"\s*[\[|<].+[\]|>]\s?escreveu\s?:"
    ),  # Outlook 2019 (pt)
    re.compile(
        r"(?m)^\s?.+\s?пользователь\s?\".+\"\s*[\[|<].+[\]|>]\s?написал\s?:"
    ),  # Outlook 2019 (ru)
    re.compile(
        r"(?m)^\s?.+\s?používateľ\s?.+\s*\([\[|<].+[\]|>]\)\s?napísal\s?:"
    ),  # Outlook 2019 (sk)
    re.compile(
        r"(?m)^\s?Den\s?.+\s?skrev\s?\".+\"\s*[\[|<].+[\]|>]\s?följande\s?:"
    ),  # Outlook 2019 (sv)
    re.compile(
        r"(?m)^\s?\".+\"\s*[\[|<].+[\]|>]\,\s?.+\s?tarihinde şunu yazdı\s?:"
    ),  # Outlook 2019 (tr)
    re.compile(
        r"(?m)^\s*-{5,8} Přeposlaná zpráva -{5,8}\s*"
    ),  # Yahoo Mail (cs), Thunderbird (cs)
    re.compile(
        r"(?m)^\s*-{5,8} Videresendt meddelelse -{5,8}\s*"
    ),  # Yahoo Mail (da), Thunderbird (da)
    re.compile(
        r"(?m)^\s*-{5,10} Weitergeleitete Nachricht -{5,10}\s*"
    ),  # Yahoo Mail (de), Thunderbird (de), HubSpot (de)
    re.compile(
        r"(?m)^\s*-{5,8} Forwarded Message -{5,8}\s*"
    ),  # Yahoo Mail (en), Thunderbird (en)
    re.compile(
        r"(?m)^\s*-{5,10} Mensaje reenviado -{5,10}\s*"
    ),  # Yahoo Mail (es), Thunderbird (es), HubSpot (es)
    re.compile(
        r"(?m)^\s*-{5,10} Edelleenlähetetty viesti -{5,10}\s*"
    ),  # Yahoo Mail (fi), HubSpot (fi)
    re.compile(r"(?m)^\s*-{5} Message transmis -{5}\s*"),  # Yahoo Mail (fr)
    re.compile(
        r"(?m)^\s*-{5,8} Továbbított üzenet -{5,8}\s*"
    ),  # Yahoo Mail (hu), Thunderbird (hu)
    re.compile(
        r"(?m)^\s*-{5,10} Messaggio inoltrato -{5,10}\s*"
    ),  # Yahoo Mail (it), HubSpot (it)
    re.compile(
        r"(?m)^\s*-{5,10} Doorgestuurd bericht -{5,10}\s*"
    ),  # Yahoo Mail (nl), Thunderbird (nl), HubSpot (nl)
    re.compile(
        r"(?m)^\s*-{5,8} Videresendt melding -{5,8}\s*"
    ),  # Yahoo Mail (no), Thunderbird (no)
    # Yahoo Mail (pl)
    re.compile(r"(?m)^\s*-{5} Przekazana wiadomość -{5}\s*"),
    re.compile(
        r"(?m)^\s*-{5,8} Mensagem reencaminhada -{5,8}\s*"
    ),  # Yahoo Mail (pt), Thunderbird (pt)
    re.compile(
        r"(?m)^\s*-{5,10} Mensagem encaminhada -{5,10}\s*"
    ...
    re.compile(r"(?m)^\s*-{5} Preposlaná správa -{5}\s*"),  # Yahoo Mail (sk)
    re.compile(
        r"(?m)^\s*-{5,10} Vidarebefordrat meddelande -{5,10}\s*"
    ),  # Yahoo Mail (sv), Thunderbird (sv), HubSpot (sv)
    re.compile(r"(?m)^\s*-{5} İletilmiş Mesaj -{5}\s*"),  # Yahoo Mail (tr)
    # Yahoo Mail (uk)
    re.compile(r"(?m)^\s*-{5} Перенаправлене повідомлення -{5}\s*"),
    # Thunderbird (fi)
    re.compile(r"(?m)^\s*-{8} Välitetty viesti \/ Fwd.Msg -{8}\s*"),
    re.compile(
        r"(?m)^\s*-{8,10} Message transféré -{8,10}\s*"
    ...
    re.compile(
        r"(?m)^\s*-{3} Treść przekazanej wiadomości -{3}\s*"
    ),  # Thunderbird (pl)
    # Thunderbird (ru)
    re.compile(r"(?m)^\s*-{8} Перенаправленное сообщение -{8}\s*"),
    re.compile(
        r"(?m)^\s*-{8} Preposlaná správa --- Forwarded Message -{8}\s*"
    ),  # Thunderbird (sk)
    re.compile(r"(?m)^\s*-{8} İletilen İleti -{8}\s*"),  # Thunderbird (tr)
    # Thunderbird (uk)
    re.compile(r"(?m)^\s*-{8} Переслане повідомлення -{8}\s*"),
    re.compile(r"(?m)^\s*-{9,10} メッセージを転送 -{9,10}\s*"),  # HubSpot (ja)
    # HubSpot (pl)
...
ORIGINAL_SUBJECT = [
    ...
    re.compile(r"(?im)^Konu\s?:(.+)"),  # Apple Mail (tr), Thunderbird (tr)
    re.compile(r"(?im)^Sujet\s?:(.+)"),  # Thunderbird (fr)
    re.compile(r"(?im)^Naslov\s?:(.+)"),  # Thunderbird (hr)
    re.compile(r"(?im)^件名：(.+)"),  # HubSpot (ja)
]

ORIGINAL_SUBJECT_LAX = [
    re.compile(r"(?i)Subject\s?:(.+)"),  # Yahoo Mail (en)
    re.compile(r"(?i)Emne\s?:(.+)"),  # Yahoo Mail (da, no)
    re.compile(r"(?i)Předmět\s?:(.+)"),  # Yahoo Mail (cs)
    re.compile(r"(?i)Betreff\s?:(.+)"),  # Yahoo Mail (de)
    re.compile(r"(?i)Asunto\s?:(.+)"),  # Yahoo Mail (es)
    re.compile(r"(?i)Aihe\s?:(.+)"),  # Yahoo Mail (fi)
    re.compile(r"(?i)Objet\s?:(.+)"),  # Yahoo Mail (fr)
    re.compile(r"(?i)Tárgy\s?:(.+)"),  # Yahoo Mail (hu)
    re.compile(r"(?i)Oggetto\s?:(.+)"),  # Yahoo Mail (it)
    re.compile(r"(?i)Onderwerp\s?:(.+)"),  # Yahoo Mail (nl)
    re.compile(r"(?i)Assunto\s?:?(.+)"),  # Yahoo Mail (pt, pt-br)
    re.compile(r"(?i)Temat\s?:(.+)"),  # Yahoo Mail (pl)
    re.compile(r"(?i)Subiect\s?:(.+)"),  # Yahoo Mail (ro)
    re.compile(r"(?i)Тема\s?:(.+)"),  # Yahoo Mail (ru, uk)
    re.compile(r"(?i)Predmet\s?:(.+)"),  # Yahoo Mail (sk)
    re.compile(r"(?i)Ämne\s?:(.+)"),  # Yahoo Mail (sv)
    re.compile(r"(?i)Konu\s?:(.+)"),  # Yahoo Mail (tr)
]
...
ORIGINAL_FROM = [
    ...
    re.compile(r"(?m)^(\s*Expeditorul\s?:(.+))$"),  # Apple Mail (ro)
    re.compile(r"(?m)^(\s*Отправитель\s?:(.+))$"),  # Apple Mail (ru)
    re.compile(
        r"(?m)^(\s*Från\s?:(.+))$"
    ...
    re.compile(r"(?m)^(\s*Від кого\s`?:(.+))$"),  # Apple Mail (uk)
    re.compile(r"(?m)^(\s*Saatja\s?:(.+))$"),  # Gmail (et)
    re.compile(r"(?m)^(\s*De la\s`?:(.+))$"),  # Gmail (ro)
    re.compile(r"(?m)^(\s*Gönderen\s?:(.+))$"),  # Gmail (tr)
    re.compile(
        r"(?m)^(\s*От\s?:(.+))$"
    ),  # Gmail (ru), New Outlook 2019 (ru), Thunderbird (ru)
    re.compile(r"(?m)^(\s*Від\s?:(.+))$"),  # Gmail (uk), Thunderbird (uk)
    re.compile(r"(?m)^(\s*Mittente\s?:(.+))$"),  # Thunderbird (it)
    re.compile(r"(?m)^(\s*Nadawca\s?:(.+))$"),  # Thunderbird (pl)
    re.compile(r"(?m)^(\s*de la\s`?:(.+))$"),  # Thunderbird (ro)
    re.compile(r"(?m)^(\s*送信元：(.+))$"),  # HubSpot (ja)
]
...
ORIGINAL_TO = [
    ...
    re.compile(
        r"(?m)^\s*Vastaanottaja\s?:(.+)$"
        # Apple Mail (fi), New Outlook 2019 (fi), Thunderbird (fi), HubSpot (fi)
    ),
    re.compile(
        r"(?m)^\s*À\s?:(.+)$"
    ),  # Apple Mail (fr), New Outlook 2019 (fr), HubSpot (fr)
    re.compile(r"(?m)^\s*Prima\s?:(.+)$"),  # Apple Mail (hr), Thunderbird (hr)
    re.compile(
        r"(?m)^\s*Címzett\s?:(.+)$"
    ),  # Apple Mail (hu), New Outlook 2019 (hu), Thunderbird (hu)
    re.compile(
    ...
    re.compile(r"(?m)^\s*Destinatarul\s?:(.+)$"),  # Apple Mail (ro)
    re.compile(
        r"(?m)^\s*Кому\s?:(.+)$"
    ),  # Apple Mail (ru, uk), New Outlook 2019 (ru), Thunderbird (ru, uk)
    re.compile(r"(?m)^\s*Pre\s?:(.+)$"),  # Apple Mail (sk), Thunderbird (sk)
    re.compile(
        r"(?m)^\s*Till\s?:(.+)$"
    ),  # Apple Mail (sv), New Outlook 2019 (sv), Thunderbird (sv)
    re.compile(r"(?m)^\s*Kime\s?:(.+)$"),  # Apple Mail (tr), Thunderbird (tr)
    re.compile(r"(?m)^\s*Pour\s?:(.+)$"),  # Thunderbird (fr)
    re.compile(r"(?m)^\s*Adresat\s?:(.+)$"),  # Thunderbird (pl)
    re.compile(r"(?m)^\s*送信先：(.+)$"),  # HubSpot (ja)
]

ORIGINAL_TO_LAX = [
    re.compile(r"(?m)\s*To\s?:(.+)$"),  # Yahook Mail (en)
    re.compile(r"(?m)\s*Komu\s?:(.+)$"),  # Yahook Mail (cs, sk)
    re.compile(r"(?m)\s*Til\s?:(.+)$"),  # Yahook Mail (da, no, sv)
    re.compile(r"(?m)\s*An\s?:(.+)$"),  # Yahook Mail (de)
    re.compile(r"(?m)\s*Para\s?:(.+)$"),  # Yahook Mail (es, pt, pt-br)
    re.compile(r"(?m)\s*Vastaanottaja\s?:(.+)$"),  # Yahook Mail (fi)
    re.compile(r"(?m)\s*À\s?:(.+)$"),  # Yahook Mail (fr)
    re.compile(r"(?m)\s*Címzett\s?:(.+)$"),  # Yahook Mail (hu)
    re.compile(r"(?m)\s*A\s?:(.+)$"),  # Yahook Mail (it)
    re.compile(r"(?m)\s*Aan\s?:(.+)$"),  # Yahook Mail (nl)
    re.compile(r"(?m)\s*Do\s?:(.+)$"),  # Yahook Mail (pl)
    re.compile(r"(?m)\s*Către\s?:(.+)$"),  # Yahook Mail (ro), Thunderbird (ro)
    re.compile(r"(?m)\s*Кому\s?:(.+)$"),  # Yahook Mail (ru, uk)
    re.compile(r"(?m)\s*Till\s?:(.+)$"),  # Yahook Mail (sv)
    re.compile(r"(?m)\s*Kime\s?:(.+)$"),  # Yahook Mail (tr)
]

ORIGINAL_REPLY_TO = [
    re.compile(r"(?m)^\s*Reply-To\s?:(.+)$"),  # Apple Mail (en)
    re.compile(r"(?m)^\s*Odgovori na\s?:(.+)$"),  # Apple Mail (hr)
    re.compile(r"(?m)^\s*Odpověď na\s?:(.+)$"),  # Apple Mail (cs)
    re.compile(r"(?m)^\s*Svar til\s?:(.+)$"),  # Apple Mail (da)
    re.compile(r"(?m)^\s*Antwoord aan\s?:(.+)$"),  # Apple Mail (nl)
    re.compile(r"(?m)^\s*Vastaus\s?:(.+)$"),  # Apple Mail (fi)
    re.compile(r"(?m)^\s*Répondre à\s?:(.+)$"),  # Apple Mail (fr)
    re.compile(r"(?m)^\s*Antwort an\s?:(.+)$"),  # Apple Mail (de)
    re.compile(r"(?m)^\s*Válaszcím\s?:(.+)$"),  # Apple Mail (hu)
    re.compile(r"(?m)^\s*Rispondi a\s?:(.+)$"),  # Apple Mail (it)
    re.compile(r"(?m)^\s*Svar til\s?:(.+)$"),  # Apple Mail (no)
    re.compile(r"(?m)^\s*Odpowiedź-do\s?:(.+)$"),  # Apple Mail (pl)
    re.compile(r"(?m)^\s*Responder A\s?:(.+)$"),  # Apple Mail (pt)
    re.compile(r"(?m)^\s*Responder a\s?:(.+)$"),  # Apple Mail (pt-br, es)
    re.compile(r"(?m)^\s*Răspuns către\s?:(.+)$"),  # Apple Mail (ro)
    re.compile(r"(?m)^\s*Ответ-Кому\s?:(.+)$"),  # Apple Mail (ru)
    re.compile(r"(?m)^\s*Odpovedať-Pre\s?:(.+)$"),  # Apple Mail (sk)
    re.compile(r"(?m)^\s*Svara till\s?:(.+)$"),  # Apple Mail (sv)
    re.compile(r"(?m)^\s*Yanıt Adresi\s?:(.+)$"),  # Apple Mail (tr)
    re.compile(r"(?m)^\s*Кому відповісти\s?:(.+)$"),  # Apple Mail (uk)
]

ORIGINAL_CC = [
    re.compile(
        r"(?m)^\*?\s*Cc\s?:\*?(.+)$"
    ),  # Apple Mail (en, da, es, fr, hr, it, pt, pt-br, ro, sk), Gmail (all locales), Outlook Live / 365 (all locales), New Outlook 2019 (da, de, en, fr, it, pt-br), Missive (en), HubSpot (de, en, es, it, nl, pt-br)
    re.compile(
        r"(?m)^\s*CC\s?:(.+)$"
    ...
    re.compile(r"(?m)^\s*Másolat\s?:(.+)$"),  # Apple Mail (hu)
    re.compile(r"(?m)^\s*Kopi\s?:(.+)$"),  # Apple Mail (no)
    re.compile(r"(?m)^\s*Dw\s?:(.+)$"),  # Apple Mail (pl)
    re.compile(
        r"(?m)^\s*Копия\s?:(.+)$"
    ...
    re.compile(r"(?m)^\s*Bilgi\s?:(.+)$"),  # Apple Mail (tr)
    re.compile(r"(?m)^\s*Копія\s?:(.+)$"),  # Apple Mail (uk),
    re.compile(r"(?m)^\s*Másolatot kap\s?:(.+)$"),  # New Outlook 2019 (hu)
    # New Outlook 2019 (sk), Thunderbird (sk)
    re.compile(r"(?m)^\s*Kópia\s?:(.+)$"),
    re.compile(r"(?m)^\s*DW\s?:(.+)$"),  # New Outlook 2019 (pl), HubSpot (pl)
    re.compile(r"(?m)^\s*Kopie \(CC\)\s?:(.+)$"),  # Thunderbird (de)
    re.compile(r"(?m)^\s*Copie à\s?:(.+)$"),  # Thunderbird (fr)
    re.compile(r"(?m)^\s*CC：(.+)$"),  # HubSpot (ja)
]

ORIGINAL_CC_LAX = [
    # Yahoo Mail (da, en, it, nl, pt, pt-br, ro, tr)
    re.compile(r"(?m)\s*Cc\s?:(.+)$"),
    re.compile(r"(?m)\s*CC\s?:(.+)$"),  # Yahoo Mail (de, es)
    re.compile(r"(?m)\s*Kopie\s?:(.+)$"),  # Yahoo Mail (cs)
    re.compile(r"(?m)\s*Kopio\s?:(.+)$"),  # Yahoo Mail (fi)
    re.compile(r"(?m)\s*Másolat\s?:(.+)$"),  # Yahoo Mail (hu)
    re.compile(r"(?m)\s*Kopi\s?:(.+)$"),  # Yahoo Mail (no)
    re.compile(r"(?m)\s*Dw\s?(.+)$"),  # Yahoo Mail (pl)
    re.compile(r"(?m)\s*Копия\s?:(.+)$"),  # Yahoo Mail (ru)
    re.compile(r"(?m)\s*Kópia\s?:(.+)$"),  # Yahoo Mail (sk)
    re.compile(r"(?m)\s*Kopia\s?:(.+)$"),  # Yahoo Mail (sv)
    re.compile(r"(?m)\s*Копія\s?:(.+)$"),  # Yahoo Mail (uk)
]
...
ORIGINAL_DATE = [
    ...
    re.compile(r"(?m)^\s*Envoyé\s?:(.+)$"),  # New Outlook 2019 (fr)
    re.compile(
        r"(?m)^\s*Fecha\s?:(.+)$"
        # Apple Mail (es), New Outlook 2019 (es), Thunderbird (es), HubSpot (es)
    ),
    ...
    re.compile(r"(?m)^\s*Dată\s?:(.+)$"),  # Apple Mail (ro), Thunderbird (ro)
    re.compile(
        r"(?m)^\s*Дата\s?:(.+)$"
    ),  # Apple Mail (ru, uk), New Outlook 2019 (ru), Thunderbird (ru, uk)
    ...
    re.compile(r"(?m)^\s*Päiväys\s?:(.+)$"),  # Thunderbird (fi)
    re.compile(r"(?m)^\s*日付：(.+)$"),  # HubSpot (ja)
]

ORIGINAL_DATE_LAX = [
    re.compile(r"(?m)\s*Datum\s?:(.+)$"),  # Yahoo Mail (cs)
    re.compile(r"(?m)\s*Sendt\s?:(.+)$"),  # Yahoo Mail (da, no)
    re.compile(r"(?m)\s*Gesendet\s?:(.+)$"),  # Yahoo Mail (de)
    re.compile(r"(?m)\s*Sent\s?:(.+)$"),  # Yahoo Mail (en)
    re.compile(r"(?m)\s*Enviado\s?:(.+)$"),  # Yahoo Mail (es, pt, pt-br)
    re.compile(r"(?m)\s*Envoyé\s?:(.+)$"),  # Yahoo Mail (fr)
    re.compile(r"(?m)\s*Lähetetty\s?:(.+)$"),  # Yahoo Mail (fi)
    re.compile(r"(?m)\s*Elküldve\s?:(.+)$"),  # Yahoo Mail (hu)
    re.compile(r"(?m)\s*Inviato\s?:(.+)$"),  # Yahoo Mail (it)
    re.compile(r"(?m)\s*Verzonden\s?:(.+)$"),  # Yahoo Mail (it)
    re.compile(r"(?m)\s*Wysłano\s?:(.+)$"),  # Yahoo Mail (pl)
    re.compile(r"(?m)\s*Trimis\s?:(.+)$"),  # Yahoo Mail (ro)
    re.compile(r"(?m)\s*Отправлено\s?:(.+)$"),  # Yahoo Mail (ru)
    re.compile(r"(?m)\s*Odoslané\s?:(.+)$"),  # Yahoo Mail (sk)
    re.compile(r"(?m)\s*Skickat\s?:(.+)$"),  # Yahoo Mail (sv)
    re.compile(r"(?m)\s*Gönderilen\s?:(.+)$"),  # Yahoo Mail (tr)
    re.compile(r"(?m)\s*Відправлено\s?:(.+)$"),  # Yahoo Mail (uk)
]
...
MAILBOX = [
    ...
    re.compile(r"^(.?)\s?\n?\s*[\[|<](.+?)[\]|>]"),
    re.compile(r"^([^\s@]+@[^\s@]+\.[^\s@,]+)"),  # "walter.sheltan@acme.com"
    re.compile(
        r"^([^;].+?)\s?\n?\s*[\[|<](.+?)[\]|>]"
